...
{
    /// <summary>
    /// it will output a list of dependecy graph nodes as flat collection
    /// Experimentation only 
    /// </summary>
    public class RestoreDGSpecTask : Microsoft.Build.Utilities.Task, ICancelableTask, IDisposable
    {
#if IS_DESKTOP
        ...
        [Output]
        public ITaskItem[] RestoreTransitiveDependencies { get; set; }
        ...
        private async Task<bool> ExecuteAsync(Common.ILogger log)
        {
            try
            {
                ...
                using (var cacheContext = new SourceCacheContext())
                {
                    ...
                    var dgspecSmall = await RestoreRunner.GetDGSpecAsync(restoreContext, _cts.Token);

                    var entries = new List<ITaskItem>();

                    var allDependencies = dgspecSmall.SelectMany(x => x.Value).SelectMany(x => x.Data.Dependencies);

                    foreach (var a in allDependencies)
                    {
                        if (!a.AutoReferenced)
                        {
                            ...
                            properties.Add("Id", a.Name);
                            properties.Add("Version", a.LibraryRange.VersionRange.PrettyPrint());


                            entries.Add(new TaskItem(a.Name, properties));
                        }
                    }

                    RestoreTransitiveDependencies = entries.ToArray();

                    //foreach (var packSpec in dgspecSmall)
                    //{
                    //    var properties = new Dictionary<string, string>();
                    //    properties.Add("ProjectUniqueName", packSpec.Key);
                    //    properties.Add("Type", "Dependency");
                    //    //properties.Add("Id", packageId);

                    //    foreach(var p in packSpec.Value)
                    //    {
                    //        p.Data.Dependencies.First().LibraryRange.VersionRange.PrettyPrint();
                    //    }

                    //    entries.Add(new TaskItem(packSpec.Key, properties));
                    //}
                    // Summary
